...
packages/
  ...
  apps-script/  Google Apps Script MCP サーバー
...
- **パーミッション**: calendar はドメインベース（self_only/internal/external）、sheets / apps-script は allowlist ベース
- **lazy auth**: MCP サーバー起動時ではなく、最初のツール呼び出し時に認証する
- **TOON**: レスポンスは TOON 形式（`@toon-format/toon`）で返す

## パッケージ間の依存

- apps-script, calendar, docs, gmail, sheets → auth（`@shivaduke28/google-mcp-auth` を `workspace:*` で参照）
...
- apps-script: permissions のユニットテスト（12テスト）
- calendar: permissions のユニットテスト（25テスト）
- docs: permissions のユニットテスト（24テスト）
